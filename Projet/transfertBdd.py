...
def get_next_id(table_name, dest_curseur):
...
def get_or_insert_realisateur(nom, prenom, page, dest_curseur, dest_conn):
    dest_curseur.execute('SELECT id FROM Realisateurs WHERE nom = ? AND prenom = ?', (nom, prenom))
    row = dest_curseur.fetchone()
    if row:
        return row[0]
    else:
        new_id = get_next_id('Realisateurs', dest_curseur)
...
def get_or_insert_acteur(nom, prenom, page, dest_curseur, dest_conn):
    dest_curseur.execute('SELECT id FROM Acteurs WHERE nom = ? AND prenom = ?', (nom, prenom))
    row = dest_curseur.fetchone()
    if row:
        return row[0]
    else:
        new_id = get_next_id('Acteurs', dest_curseur)
...
def get_or_insert_theme(nom, dest_curseur, dest_conn):
    dest_curseur.execute('SELECT id FROM Themes WHERE nom = ?', (nom,))
    row = dest_curseur.fetchone()
    if row:
        return row[0]
    else:
        new_id = get_next_id('Themes', dest_curseur)
...
def get_or_insert_genre(nom, dest_curseur, dest_conn):
    dest_curseur.execute('SELECT id FROM Genres WHERE nom = ?', (nom,))
    row = dest_curseur.fetchone()
    if row:
        return row[0]
    else:
        new_id = get_next_id('Genres', dest_curseur)
        dest_curseur.execute('INSERT INTO Genres (id, nom) VALUES (?, ?)', (new_id, nom))
        dest_conn.commit()
        return new_id

# Fonction pour transférer les données d'une base source vers la base destination
def transfer_data(source_db, dest_curseur, dest_conn):
    ...
    for nom, prenom, page in realisateurs:
        get_or_insert_realisateur(nom, prenom, page, dest_curseur, dest_conn)
    ...
    for film in films:
        ...
        if row:
            ...
            if realisateur_row:
                ...
                realisateur_id = get_or_insert_realisateur(nom_realisateur, prenom_realisateur, None, dest_curseur, dest_conn)
    ...
    for film, acteur in jouer:
        ...
        if acteur_row:
            ...
            acteur_id = get_or_insert_acteur(nom_acteur, prenom_acteur, None, dest_curseur, dest_conn)
    ...
    for film, genre in genre_films:
        ...
        if genre_row:
            (nom_genre,) = genre_row
            genre_id = get_or_insert_genre(nom_genre, dest_curseur, dest_conn)
    ...
    for film, theme in theme_films:
        ...
        if theme_row:
            (nom_theme,) = theme_row
            theme_id = get_or_insert_theme(nom_theme, dest_curseur, dest_conn)
    ...
    # Fermeture de la connexion source
    source_conn.close()
...
source_dbs = [f'films2010s copy {i}.sqlite' for i in range(2, 11)]

# Transférer les données de chaque base de données source
for source_db in source_dbs:
    transfer_data(source_db, dest_curseur, dest_conn)

# Fermeture de la connexion destination
dest_conn.close()
