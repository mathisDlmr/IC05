...
conn <- dbConnect(SQLite(), "films2010s copy 5.sqlite")

Sys.setenv(JAVA_HOME = "C:\\Program Files\\Java\\jre-1.8\\bin")
driver <- rsDriver(browser = "firefox", port = 4436L, verbose = F, chromever = NULL)
remDr <- driver[["client"]]
...
i <- 184994 #Clé primaire incrémentée à la main pour la stocker en variable et reprendre le script de n'importe où
while (i < 248961) {
    tryCatch({
